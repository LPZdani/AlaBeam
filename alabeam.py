...
with st.sidebar:
    ...
    st.caption(
        "**LGBM:** `model_max_displacement_HGB.joblib`, `model_max_stress_HGB.joblib`  \n"
...
# Validación en tiempo real de apoyos
supports = [support_L, support_R]
active_supports = [s for s in supports if s is not None]

if len(active_supports) == 1 and active_supports[0] == "simple":
    st.error("⚠️ **Configuración inválida:** Una viga con un solo apoyo simple es un mecanismo. "
            "Para un solo apoyo, debe ser empotrado.")
elif len(active_supports) == 0:
    st.warning("⚠️ **Advertencia:** La viga necesita al menos un apoyo para ser analizada.")
elif len(active_supports) == 1 and active_supports[0] == "clamped":
    st.success("✅ **Configuración válida:** Viga en voladizo (cantilever) con apoyo empotrado.")
...
def predict_lgbm(df_feat: pd.DataFrame, models_path: Path, safety_factor: float = 1.1):
    disp_path = models_path / "model_max_displacement_HGB.joblib"
    stress_path = models_path / "model_max_stress_HGB.joblib"
...
def predict_neural(df_feat: pd.DataFrame, models_path: Path, safety_factor: float = 1.1):
    ...
    for p in [disp_model, stress_model, disp_scaler, stress_scaler]:
        if not p.exists():
    ...
    # 2) Reindex estricto con esas columnas
    ...
    # 5) Carga escaladores - manejar ambos formatos (dict y objeto directo)
    sc_d_raw = joblib.load(disp_scaler)
    sc_s_raw = joblib.load(stress_scaler)
    
    # Si los scalers están guardados como diccionarios (formato models_neural)
    if isinstance(sc_d_raw, dict):
        sc_d = sc_d_raw['scaler']
        # También podríamos usar las feature_cols si están disponibles
        # feature_cols_d = sc_d_raw.get('feature_cols', None)
    else:
        # Si están guardados directamente (formato models_neural_2)
        sc_d = sc_d_raw
        
    if isinstance(sc_s_raw, dict):
        sc_s = sc_s_raw['scaler']
        # feature_cols_s = sc_s_raw.get('feature_cols', None)
    else:
        sc_s = sc_s_raw
...
if btn:
    try:
        # Validación de apoyos para evitar mecanismos
        supports = [support_L, support_R]
        active_supports = [s for s in supports if s is not None]
        
        if len(active_supports) == 1 and active_supports[0] == "simple":
            st.error("⚠️ **Error de configuración:** Una viga con un solo apoyo simple es un mecanismo y no puede ser analizada. "
                    "Si solo hay un apoyo, debe ser empotrado (clamped) para proporcionar estabilidad estructural.")
            st.stop()
        
        if len(active_supports) == 0:
            st.error("⚠️ **Error de configuración:** La viga debe tener al menos un apoyo para ser analizada.")
            st.stop()
